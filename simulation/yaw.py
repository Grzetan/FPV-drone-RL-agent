...
class RadioMasterJoystick(BaseJoystick):
    """Simplified joystick controller that only reads yaw axis."""
    
    def get_action_array(self):
        """
        Get yaw control value as numpy array for drone control.
        
        Returns:
            np.array: Array with [yaw] value in [-1, 1] range
        """
        controls = self.get_controls()
        return np.array([controls['yaw']], dtype=np.float32)


class DroneEnv(HoverDroneEnv):
    """
    Yaw-only control environment.
    Inherits from hover DroneEnv but constrains action space to yaw only.
    Roll, pitch, and throttle are hardcoded.
    """
    ...
    def __init__(self, render=False):
        ...
        # Override action space to be 1D (yaw only)
        self.action_space = spaces.Box(low=-1.0, high=1.0, shape=(1,), dtype=np.float32)
        
        # Override observation space: [attitude_xyz (3), sphere_center_xy (2), angular_velocity_xyz (3), last_4_yaw_actions (4)]
        # Total: 3 + 2 + 3 + 4 = 12
        ...
        self.hardcoded_throttle = -1.0
        
        # Override action history to store scalar yaw values instead of 4D arrays
    ...
    def create_observation(self, sensors):
        ...
        # Last 4 yaw actions as array (scalars not 4D arrays)
        last_4_yaw_actions = np.array(list(self.action_history), dtype=np.float32)

        return np.concatenate(
            (attitude_data, sphere_center, angular_velocity, last_4_yaw_actions),
            dtype=np.float32,
        )
    ...
    def reset(self, seed=None, options=None):
        ...
        # Reset action history buffer with scalar yaw values
    ...
    def step(self, action):
        """Override to handle 1D action (yaw only) and construct full 4D action."""
        # Extract yaw from action (1D array)
        yaw_action = float(action[0])
        
        # Store the raw yaw action in history
        self.action_history.append(yaw_action)
    ...
    def test_env_with_joystick(self, joystick_index=0):
        """Override to display yaw-only control instructions."""
        try:
            # Initialize RadioMaster joystick
            joystick = RadioMasterJoystick(joystick_index)
            
            print("RadioMaster Pocket Joystick Controls:")
            print("  Axis 3: Yaw (rotation) - ONLY CONTROL")
            print("  Roll, Pitch: Hardcoded to 0")
            print("  Throttle: Hardcoded to -1")
            ...
            iteration = 0
            while True:
                # Get joystick controls (only yaw)
                ...
                if iteration % 10 == 0:
                    ...
                    print(f"  Yaw action: {action[0]:.3f}")
                ...
                cv2.imshow("RadioMaster Pocket Control - Yaw Only", frame)
...
if __name__ == "__main__":
    ...
    print("Environment loaded. Starting RadioMaster Pocket control (Yaw only)...")

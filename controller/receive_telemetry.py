...
def handleCrsfPacket(ptype, data):
    if ptype == PacketsTypes.RADIO_ID and data[5] == 0x10:
        ...
        pitch = int.from_bytes(data[3:5], byteorder='big', signed=True) / 10000.0 * 57.2958
        roll = int.from_bytes(data[5:7], byteorder='big', signed=True) / 10000.0 * 57.2958
        yaw = int.from_bytes(data[7:9], byteorder='big', signed=True) / 10000.0 * 57.2958
...
with serial.Serial("/dev/ttyACM0", 460800, timeout=2) as ser:
    input = bytearray()
    while True:
        if ser.in_waiting > 0:
